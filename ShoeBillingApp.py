...
class ShoeBillingApp:
    def __init__(self, root):
        ...
        win_w = min(1550, int(sw * 0.95))
        win_h = min(950, int(sh * 0.92))
        self.root.geometry(f"{win_w}x{win_h}+{(sw-win_w)//4}+{(sh-win_h)//4}")
        ...
        # --- 打印字体大小设置 ---
        self.print_font_size = self._load_print_font_size()  # 销售单基础字号 (px)
        self.quote_font_size = self._load_quote_font_size()  # 报价单基础字号 (px)
        
        # --- 选项卡字体调节临时存储 ---
        self._font_sizes = {}  # {container_id: current_size}
        self._tab_font_state = {}  # {tab_name: {container_id: size}}
    ...
    # --- 打印字体大小配置持久化 ---
    def _get_cfg_path(self, name):
        return os.path.join(BASE_DIR, name)

    def _load_print_font_size(self):
        try:
            p = self._get_cfg_path("print_font_size.cfg")
            if os.path.exists(p):
                v = int(open(p, "r", encoding="utf-8").read().strip())
                if 20 <= v <= 100:
                    return v
        except Exception:
            pass
    ...
    def _save_print_font_size(self, size):
        try:
            with open(self._get_cfg_path("print_font_size.cfg"), "w", encoding="utf-8") as f:
                f.write(str(size))
        except Exception:
            pass

    def _load_quote_font_size(self):
        try:
            p = self._get_cfg_path("quote_font_size.cfg")
            if os.path.exists(p):
                v = int(open(p, "r", encoding="utf-8").read().strip())
                if 20 <= v <= 100:
                    return v
    ...
    def _save_quote_font_size(self, size):
        try:
            with open(self._get_cfg_path("quote_font_size.cfg"), "w", encoding="utf-8") as f:
                f.write(str(size))
        except Exception:
            pass
    ...
    def open_print_settings(self):
        ...
        tk.Label(win, text="🖨️ 打印字体大小设置", font=("微软雅黑", 14, "bold"), bg="#f0f2f5").pack(pady=(20, 10))
        
        tk.Label(win, text="选择字体大小（px）:", font=("微软雅黑", 10), bg="#f0f2f5").pack(pady=5)
        
        font_var = tk.IntVar(value=self.print_font_size)
        font_spin = ttk.Spinbox(win, from_=10, to=48, width=10, textvariable=font_var, font=("微软雅黑", 12))
        font_spin.pack(pady=5)
        
        def save_font():
            try:
                size = int(font_var.get())
                self.print_font_size = size
                self.quote_font_size = size
                self._save_print_font_size(size)
                self._save_quote_font_size(size)
                messagebox.showinfo("✅ 保存成功", f"字体大小已设置为 {size}px\n下次打印将生效。")
                win.destroy()
            except:
                messagebox.showerror("错误", "请输入有效的数字！")
        
        tk.Button(win, text="💾 保存", command=save_font, bg="#1677ff", fg="white",
                  font=("微软雅黑", 11, "bold"), relief="flat", padx=30, pady=8).pack(pady=(15, 10))
        
        tk.Button(win, text="❌ 取消", command=win.destroy, bg="#d9d9d9", fg="#333",
                  font=("微软雅黑", 11, "bold"), relief="flat", padx=30, pady=8).pack(pady=(0, 10))
    ...
    def import_products(self):
        ...
        if path:
            try:
                ...
                # --- 核心改进：自动识别编码 ---
                # 依次尝试：带BOM的UTF-8, GBK(WPS默认), 标准UTF-8
                ...
                if content_lines is None:
                    raise Exception("文件编码不受支持，请将CSV保存为UTF-8或GBK格式。")
                # -----------------------------
    ...
    def gen_quotation_html(self, d):
        ...
        for i, it in enumerate(d['items']):
            ...
            <tr style="text-align:center; height:100px;">
                ...
                <td style="border:1px solid #ddd; padding:10px; font-size:var(--font-size); white-space:nowrap;">{it['size']}</td>
        ...
        <head>
            ...
            <style>
                :root {{ --font-size: {self.quote_font_size}px; }}
                body {{ font-family: 'Segoe UI', Arial, sans-serif; padding: calc(var(--font-size) * 2); color: #333; background: #f5f5f5; }}
                .quotation-container {{ background: white; padding: calc(var(--font-size) * 2); box-shadow: 0 2px 8px rgba(0,0,0,0.1); width: 100%; max-width: 100%; }}
                .title {{ font-size: calc(var(--font-size) * 1.05); font-weight: bold; text-align: center; margin-bottom: 5px; text-transform: uppercase; letter-spacing: 2px; }}
                .title-sender {{ text-align: center; font-size: calc(var(--font-size) * 0.29); color: #666; margin-bottom: 8px; white-space: nowrap; line-height: 1.2; }}
                .line {{ border-bottom: 3px solid #000; margin-bottom: 12px; }}
                .meta-row {{ display: flex; flex-wrap: nowrap; align-items: center; justify-content: flex-start; margin-bottom: calc(var(--font-size) * 0.3); padding: calc(var(--font-size) * 0.3) calc(var(--font-size) * 0.6); border: 1px solid #eee; background: #fafafa; border-radius: 4px; overflow: hidden; }}
                .meta-label {{ font-weight: bold; color: #1976d2; margin-right: 12px; border-right: 2px solid #1976d2; padding-right: 10px; flex-shrink: 0; font-size: calc(var(--font-size) * 0.4); min-width: 110px; }}
                .meta-container {{ display: flex; flex-wrap: nowrap; flex-grow: 1; align-items: center; overflow: hidden; }}
                .meta-item {{ margin-right: 15px; white-space: nowrap; flex-shrink: 0; }}
                .meta-item b {{ color: #777; font-size: calc(var(--font-size) * 0.75); margin-right: 4px; border-left: 1px solid #ddd; padding-left: 8px; }}
                .meta-item:first-child b {{ border-left: none; padding-left: 0; }}
                table {{ width: 100%; border-collapse: collapse; margin-top: 10px; font-size: var(--font-size); table-layout: auto; }}
                th {{ background: #333; color: #fff; padding: 15px 5px; text-transform: uppercase; border: 1px solid #333; font-size: var(--font-size); }}
                .footer-note {{ margin-top: calc(var(--font-size) * 2); padding: calc(var(--font-size) * 1); font-size: calc(var(--font-size) * 0.93); color: #666; font-style: italic; }}
                /* 打印页码设置 */
                ...
                @media print {{ 
                    ...
                    .quotation-container {{ box-shadow: none; padding: 20px; margin: 0 auto; }}
                    @page {{ margin: 1.5cm; }}
                    .page-numbering {{ 
                        display: block; 
        ...
        <body>
            <div class="quotation-container">
                ...
                <table>
                    <thead>
                        <tr>
                            <th style="width:auto; min-width:100px; max-width:180px">{t['photo']}</th>
                            <th style="width:auto; min-width:80px">{t['no']}</th>
                            <th style="width:auto; min-width:80px">{t['size']}</th>
                            <th style="width:auto; min-width:80px">{t['color']}</th>
                            <th style="width:auto; min-width:60px">{t['moq']}</th>
                            <th style="width:auto; min-width:60px">{t['pcs_ctn']}</th>
                            <th style="width:auto; min-width:60px">{t['qty']}</th>
                            <th style="width:auto; min-width:80px">{t['price']}</th>
                        </tr>
                    </thead>
                    <tbody>
                        {rows}
            ...
            <div class="no-print" style="position:fixed; bottom:150px; right:30px; background:white; padding:6px 12px; border-radius:8px; box-shadow: 0 2px 8px rgba(0,0,0,0.15); z-index:999; display:flex; align-items:center; gap:8px;">
                ...
                <input type="range" id="fontSizeSlider" min="15" max="35" value="{self.quote_font_size}" step="1" 
                       oninput="document.documentElement.style.setProperty('--font-size', this.value+'px');document.getElementById('fontSizeLabel').textContent=this.value;" 
                       style="width:140px; cursor:pointer; accent-color:#1677ff;">
            </div>
            <script>
        ...
        for i in range(0, len(items), cols):
            ...
            for it in row_items:
                ...
                <div style="flex: 0 0 {width_style}; min-width: 0; text-align: center; border: 1px solid #e0e0e0; padding: 20px; background: #fff; border-radius: 4px;">
                    ...
                    <div style="font-size: calc(var(--font-size) * 0.72); font-weight: bold; margin-bottom: 8px; color: #333; text-align: left; padding-left: 5px;">
                        {t['no']}: {it.get('no', '--')}
                    </div>
                    <div style="font-size: calc(var(--font-size) * 0.66); margin-bottom: 5px; color: #666; text-align: left; padding-left: 5px;">
                        {t['color']}: {it.get('color', '--')}
                    </div>
                    <div style="font-size: calc(var(--font-size) * 0.66); margin-bottom: 5px; color: #666; text-align: left; padding-left: 5px;">
                        {t['size']}: {it.get('size', '--')}
                    </div>
                    <div style="font-size: calc(var(--font-size) * 0.78); font-weight: bold; margin-top: 10px; margin-bottom: 8px; color: #d32f2f; text-align: left; padding-left: 5px;">
                        {t['price']}: ¥{float(it.get('price', 0)):.2f}
                    </div>
                    <div style="font-size: calc(var(--font-size) * 0.66); margin-top: 8px; margin-bottom: 4px; color: #555; text-align: left; padding-left: 5px; border-top: 1px solid #eee; padding-top: 8px;">
                        Packing: {packing}
                    </div>
                    <div style="font-size: calc(var(--font-size) * 0.66); margin-bottom: 4px; color: #555; text-align: left; padding-left: 5px;">
        ...
        <head>
            ...
            <style>
                ...
                * {{ box-sizing: border-box; }}
                ...
                .title {{
                    font-size: calc(var(--font-size) * 1.1);n                    font-weight: bold;
                    text-align: center;
                    margin-bottom: 5px;
                ...
                .title-sender {{ text-align: center; font-size: calc(var(--font-size) * 0.26); color: #666; margin-bottom: 8px; white-space: nowrap; line-height: 1.2; }}
                .line {{ border-bottom: 3px solid #000; margin-bottom: 12px; }}
                .meta-row {{ display: flex; flex-wrap: nowrap; align-items: center; justify-content: flex-start; margin-bottom: calc(var(--font-size) * 0.3); padding: calc(var(--font-size) * 0.3) calc(var(--font-size) * 0.6); border: 1px solid #eee; background: #fafafa; border-radius: 4px; overflow: hidden; }}
                .meta-label {{ font-weight: bold; color: #1976d2; margin-right: 12px; border-right: 2px solid #1976d2; padding-right: 10px; flex-shrink: 0; font-size: calc(var(--font-size) * 0.4); min-width: 110px; }}
                .meta-container {{ display: flex; flex-wrap: nowrap; flex-grow: 1; align-items: center; overflow: hidden; }}
    ...
    def setup_billing_ui(self):
        ...
        main_wrapper.pack(fill="both", expand=True, padx=8, pady=4)
        ...
        f_header_con = tk.Frame(left_top_container, bg="#ffffff", pady=2)
        ...
        f_receiver_row = tk.LabelFrame(f_header_con, text=" 📦 收货信息 ", 
                                      font=self.fonts['body_bold'], padx=6, pady=3,
                                      bg="#ffffff", fg="#1677ff", relief="flat", bd=1,
                                      highlightthickness=1, highlightbackground="#e0e0e0")
        f_receiver_row.pack(fill="x", pady=(0, 2))
        
        f_receiver = tk.Frame(f_receiver_row, bg="#ffffff")
        f_receiver.pack(fill="x", padx=4, pady=2)
        
        # 收货信息字段：客户、唛头、业务电话、收货地址、备注 - 一行显示（去掉英文）
        r_fields = [("客户:", "client", 8), ("唛头:", "mark", 8), 
                   ("业务电话:", "phone", 10), ("收货地址:", "address", 12),
                   ("备注:", "note", 10)]
        ...
        f_sender_row = tk.LabelFrame(f_header_con, text=" 🚚 发货信息 ", 
                                     ...
                                     highlightthickness=1, highlightbackground="#e0e0e0")
        f_sender_row.pack(fill="x", pady=(0, 2))
        
        f_sender = tk.Frame(f_sender_row, bg="#ffffff")
        f_sender.pack(fill="x", padx=4, pady=2)
        
        # 发货信息字段：发货人、联系电话、发货地址 - 一行显示（去掉英文）
        s_fields = [("发货人:", "sender", 8), ("联系电话:", "sender_phone", 10), 
                   ("发货地址:", "sender_address", 12)]
        ...
        f_settle.pack(fill="x", padx=4, pady=2)
        
        self.fin_ents = {}
        fin_fields = [("运费:", "shipping", 6), ("定金:", "deposit", 6), 
                     ("已收:", "paid", 6)]
        ...
        col_widths = [30, 100, 70, 65, 50, 50, 55, 65, 70]
        for cid, head, cw in zip(self.tree_cart["columns"], heads, col_widths):
            self.tree_cart.heading(cid, text=head); self.tree_cart.column(cid, width=cw, anchor="center")
        ...
        # 底部控制区：两行布局，避免按钮被挤出屏幕
        btn_container = tk.Frame(cart_card, bg="#ffffff")
        ...
        self.print_product_img_var = tk.BooleanVar(value=True)
        
        # 第一行：复选框 + 语言选择
        row1 = tk.Frame(btn_container, bg="#ffffff")
        row1.pack(fill="x", pady=(0, 4))
        tk.Checkbutton(row1, text="显示价格金额", variable=self.show_price_var,
                      bg="#ffffff", font=self.fonts['body']).pack(side="left", padx=(0, 8))
        tk.Checkbutton(row1, text="同步生成备货单", variable=self.sync_stock_prep_var,
                      bg="#ffffff", font=self.fonts['body'], fg="#2ecc71").pack(side="left", padx=(0, 8))
        tk.Checkbutton(row1, text="打印商品图片", variable=self.print_product_img_var,
                      bg="#ffffff", font=self.fonts['body'], fg="#1677ff").pack(side="left", padx=(0, 8))
        tk.Label(row1, text="|  打印语言:", bg="#ffffff", fg="#999", font=self.fonts['body']).pack(side="left", padx=(5, 4))
        tk.Radiobutton(row1, text="English", variable=self.print_lang_var, value="en", bg="#ffffff", font=self.fonts['body']).pack(side="left")
        tk.Radiobutton(row1, text="中文", variable=self.print_lang_var, value="zh", bg="#ffffff", font=self.fonts['body']).pack(side="left", padx=(0, 8))
        
        # 第二行：打印按钮（全宽）
        row2 = tk.Frame(btn_container, bg="#ffffff")
        row2.pack(fill="x")
        tk.Button(row2, text="💾 保存并打印", bg="#1677ff", fg="white", 
                 font=self.fonts['button'], height=1, command=self.save_print, relief="flat", cursor="hand2",
                 activebackground="#0958d9", padx=20, pady=8).pack(fill="x", expand=True)
        ...
        tk.Label(f_pick_search, text="货号搜索:", font=self.fonts['body_bold'], 
                bg="#ffffff", fg="#1f1f1f").pack(side="left", padx=(0, 10))
        self.ent_bill_pick_search = tk.Entry(f_pick_search, width=16, font=self.fonts['body'],
                                            relief="solid", bd=1, highlightthickness=2,
                                            highlightbackground="#e0e0e0", highlightcolor="#1677ff"); 
    ...
    def gen_invoice_html(self, d):
        ...
        # 获取打印商品图片选项
        show_img = True
        if hasattr(self, 'print_product_img_var'):
            show_img = self.print_product_img_var.get()
        ...
        if img_mode is not None and self.invoice_img_mode.get() == "custom":
            try:
                ...
                pass
        row_h = img_h + 10 if show_img else 26
        ...
        for it in d['items']:
            ...
            img_td = f'<td style="border:1px solid #ddd; padding:8px;"><img src="data:image/jpeg;base64,{it.get("img", "")}" style="height:{img_h}px; max-width:{img_w}px; object-fit:contain;"></td>' if show_img else ''
        ...
        # 表头处理
        photo_th = f'<th style="width:auto; min-width:100px; max-width:180px">{t["photo"]}</th>' if show_img else ''
        ...
        if show_price:
            summary_html += f"""
                ...
                <div class="summary-item"><b>{t['total']}:</b> <span style="font-weight:bold; color:#f57c00; font-size:calc(var(--font-size) * 1.1);">¥{total_all:.2f}</span></div>
        ...
        <head>
            ...
            <script src="https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js"></script>
            <style>
                :root {{ --print-size: {self.print_font_size}px; }}
                * {{ font-family: 'Segoe UI', 'Microsoft YaHei', Arial, sans-serif; }}
                body {{ font-size: var(--print-size); padding: calc(var(--font-size) * 2); color: #333; background: #f5f5f5; }}
                .invoice-container {{ background: white; padding: calc(var(--font-size) * 2); box-shadow: 0 2px 8px rgba(0,0,0,0.1); width: 100%; max-width: 100%; }}
                .header-table {{ width: 100%; margin-bottom: 20px; border:none; }}
                .title {{ font-size: 1.05em; font-weight: bold; text-align: center; margin-bottom: 5px; text-transform: uppercase; letter-spacing: 2px; }}
                .title-sender {{ text-align: center; font-size: 0.24em; color: #666; margin-bottom: 8px; white-space: nowrap; line-height: 1.2; }}
                .line {{ border-bottom: 3px solid #000; margin-bottom: 12px; }}
                .meta-row {{ display: flex; flex-wrap: nowrap; align-items: center; justify-content: flex-start; margin-bottom: calc(var(--font-size) * 0.3); padding: calc(var(--font-size) * 0.3) calc(var(--font-size) * 0.6); border: 1px solid #eee; background: #fafafa; border-radius: 4px; overflow: hidden; }}
                .meta-label {{ font-weight: bold; color: #1976d2; margin-right: 12px; border-right: 2px solid #1976d2; padding-right: 10px; flex-shrink: 0; font-size: 0.4em; min-width: 110px; }}
                .meta-container {{ display: flex; flex-wrap: nowrap; flex-grow: 1; align-items: center; overflow: hidden; }}
                .meta-item {{ margin-right: 15px; white-space: nowrap; flex-shrink: 0; }}
                .meta-item b, .meta-item {{ font-size: 0.75em; color: #777; margin-right: 4px; border-left: 1px solid #ddd; padding-left: 8px; }}
                .meta-item:first-child b {{ border-left: none; padding-left: 0; }}
                table {{ width: 100%; border-collapse: collapse; margin-top: 5px; font-size: var(--font-size); table-layout: auto; }}
                th {{ background: #333; color: #fff; padding: 6px 5px; text-transform: uppercase; border: 1px solid #333; font-size: var(--font-size); }}
                td {{ border: 1px solid #ddd; padding: 10px 8px; font-size: inherit; }}
                tr {{ page-break-inside: avoid; }}
                .summary-row {{ display: flex; flex-wrap: nowrap; align-items: center; justify-content: space-between; margin-top: calc(var(--font-size) * 1.5); padding: calc(var(--font-size) * 0.8) calc(var(--font-size) * 1); border: 2px solid #f57c00; background: #fff8e1; border-radius: 4px; overflow: hidden; }}
                ...
                .summary-item span {{ font-size: inherit; }}
                .summary-highlight {{ font-weight: bold; font-size: calc(var(--font-size) * 1.2); color: #d32f2f; }}
                .note-box {{ margin-top: calc(var(--font-size) * 1.5); padding: calc(var(--font-size) * 1); border: 1px solid #ddd; background: #f9f9f9; font-size: calc(var(--font-size) * 0.85); }}
                .note-box b {{ font-size: inherit; }}
                /* 打印页码设置 */
                body {{ counter-reset: page; }}
                .page-numbering {{
                    display: none;
                ...
                @media print {{ 
                    ...
                    .invoice-container {{ box-shadow: none; padding: 20px; margin: 0 auto; width: auto; max-width: 100%; }}
                    @page {{ margin: 1.5cm; }}
        ...
        <body>
            <div class="invoice-wrapper" style="padding:20px; background:#ffffff;">
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th style="width:auto; min-width:80px">{t['size']}</th>
                            <th style="width:auto; min-width:80px">{t['color']}</th>
                            <th style="width:auto; min-width:60px">{t['ctns']}</th>
                            <th style="width:auto; min-width:60px">{t['pcs_ctn']}</th>
                            <th style="width:auto; min-width:60px">{t['qty']}</th>
                            {price_th}
                            {amount_th}
            ...
            <div class="no-print" style="position:fixed; bottom:150px; right:30px; background:white; padding:6px 12px; border-radius:8px; box-shadow: 0 2px 8px rgba(0,0,0,0.15); z-index:999; display:flex; align-items:center; gap:8px;">
                ...
                <input type="range" id="fontSizeSlider" min="15" max="35" value="{self.print_font_size}" step="1" 
                       oninput="document.documentElement.style.setProperty('--print-size', this.value+'px');document.getElementById('fontSizeLabel').textContent=this.value;" 
                       style="width:140px; cursor:pointer; accent-color:#1677ff;">
            </div>
            <script>
            async function copyImages(event) {{
    ...
    def copy_cart_item(self):
        """右键复制当前选中的发货清单记录，复制后可双击编辑尺码/颜色/图片等"""
        sel = self.tree_cart.selection()
        if sel:
            idx = int(self.tree_cart.item(sel[0], "values")[0]) - 1
            if 0 <= idx < len(self.cart_items):
                import copy
                new_item = copy.deepcopy(self.cart_items[idx])
                # 插入到原记录后面
    ...
    def gen_stock_prep_html(self, d, lang=None):
        ...
        for i, it in enumerate(d['items']):
            ...
            <tr style="text-align:center; height:100px;">
                ...
                <td style="border:1px solid #ddd; padding:8px;">{it['no']}</td>
                <td style="border:1px solid #ddd; padding:10px; font-size:var(--font-size); white-space:nowrap;">{size_display}</td>
                <td style="border:1px solid #ddd; padding:8px;">{it.get('color','')}</td>
        ...
        <head>
            ...
            <script src="https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js"></script>
            <style>
                :root {{ --font-size: {self.print_font_size}px; }}
                body {{ font-family: 'Segoe UI', Arial, sans-serif; padding: calc(var(--font-size) * 2); color: #333; }}
                .stock-prep-container {{ background: white; padding: calc(var(--font-size) * 1); }}
                .title {{ font-size: calc(var(--font-size) * 1.0); font-weight: bold; text-align: center; margin-bottom: 10px; text-transform: uppercase; letter-spacing: 2px; }}
                .line {{ border-bottom: 3px solid #000; margin-bottom: calc(var(--font-size) * 1.5); }}
                .meta-info {{ font-size: calc(var(--font-size) * 0.67); margin-bottom: 10px; line-height: 1.6; }}
                table {{ width: 100%; border-collapse: collapse; margin-top: 10px; font-size: var(--font-size); table-layout: auto; }}
                th {{ background: #333; color: #fff; padding: 15px 5px; text-transform: uppercase; border: 1px solid #333; font-size: var(--font-size); }}
                .summary-box {{ margin-top: calc(var(--font-size) * 1.5); padding: calc(var(--font-size) * 1); border: 2px solid #333; background: #f9f9f9; display: inline-block; min-width: 300px; }}
                .summary-item {{ font-size: calc(var(--font-size) * 1.06); font-weight: bold; margin: 5px 0; }}
                @media print {{ .no-print {{ display: none; }} body {{ padding: 20px; margin: 0; background: white; }} .stock-prep-container {{ box-shadow: none; padding: 20px; margin: 0 auto; }} @page {{ margin: 1.5cm; }} }}
            </style>
        </head>
        <body>
    ...
    def on_cart_item_edit(self, event):
        ...
        tk.Label(f_in, text="码段 (Size):", font=self.fonts['label'], bg="#ffffff", 
                fg="#1f1f1f").grid(row=row_idx, column=0, padx=12, pady=8, sticky="e")
        e_size = tk.Entry(f_in, font=self.fonts['body'], width=20, relief="solid", bd=1,
                         highlightthickness=2, highlightbackground="#e0e0e0", highlightcolor="#1677ff")
        e_size.insert(0, str(it.get('size', '')))
        e_size.grid(row=row_idx, column=1, padx=12, pady=8, ipady=5)
        row_idx += 1
        
        # 颜色 (Color) — 新增
        tk.Label(f_in, text="颜色 (Color):", font=self.fonts['label'], bg="#ffffff", 
                fg="#1f1f1f").grid(row=row_idx, column=0, padx=12, pady=8, sticky="e")
        e_color = tk.Entry(f_in, font=self.fonts['body'], width=20, relief="solid", bd=1,
                          highlightthickness=2, highlightbackground="#e0e0e0", highlightcolor="#1677ff")
        e_color.insert(0, str(it.get('color', '')))
        e_color.grid(row=row_idx, column=1, padx=12, pady=8, ipady=5)
        row_idx += 1
        
        # 每箱数量 (Pcs)
        tk.Label(f_in, text="每箱数量 (Pcs):", font=self.fonts['label'], bg="#ffffff", 
                fg="#1f1f1f").grid(row=row_idx, column=0, padx=12, pady=8, sticky="e")
        e_pcs = tk.Entry(f_in, font=self.fonts['body'], width=20, relief="solid", bd=1,
                         highlightthickness=2, highlightbackground="#e0e0e0", highlightcolor="#1677ff")
        e_pcs.insert(0, str(it.get('pcs', '')))
        e_pcs.grid(row=row_idx, column=1, padx=12, pady=8, ipady=5)
        row_idx += 1
        
        # 箱数 (Ctns)
        tk.Label(f_in, text="箱数 (Ctns):", font=self.fonts['label'], bg="#ffffff", 
                fg="#1f1f1f").grid(row=row_idx, column=0, padx=12, pady=8, sticky="e")
        e_ctns = tk.Entry(f_in, font=self.fonts['body'], width=20, relief="solid", bd=1,
                         highlightthickness=2, highlightbackground="#e0e0e0", highlightcolor="#1677ff")
        e_ctns.insert(0, str(it['ctns']))
        e_ctns.grid(row=row_idx, column=1, padx=12, pady=8, ipady=5)
        row_idx += 1
        
        # 单价 (Price)
        tk.Label(f_in, text="单价 (Price):", font=self.fonts['label'], bg="#ffffff", 
                fg="#1f1f1f").grid(row=row_idx, column=0, padx=12, pady=8, sticky="e")
        e_price = tk.Entry(f_in, font=self.fonts['body'], width=20, relief="solid", bd=1,
                          highlightthickness=2, highlightbackground="#e0e0e0", highlightcolor="#1677ff")
        e_price.insert(0, str(it['price']))
        e_price.grid(row=row_idx, column=1, padx=12, pady=8, ipady=5)
        row_idx += 1
        
        # 图片 (Image) — 新增
        img_preview_lbl = None
        temp_img_data = {"val": it.get('img', '')}  # 用dict以便在闭包中修改
        
        img_f = tk.Frame(f_in, bg="#ffffff")
        img_f.grid(row=row_idx, column=0, columnspan=2, padx=12, pady=8, sticky="w")
        tk.Label(img_f, text="商品图片:", font=self.fonts['label'], bg="#ffffff", 
                fg="#1f1f1f").pack(side="left")
        
        def select_img():
            p = filedialog.askopenfilename(filetypes=[("图片文件", "*.jpg *.jpeg *.png *.bmp *.gif")])
            if p:
                try:
                    img = Image.open(p)
                    buf = BytesIO()
                    img.convert("RGB").save(buf, format="JPEG", quality=95)
                    temp_img_data["val"] = base64.b64encode(buf.getvalue()).decode("utf-8")
                    update_img_preview()
                except Exception:
        ...
        def paste_img():
            ...
            if img:
                ...
                messagebox.showinfo("提示", "剪贴板没有可识别的图片。")
        ...
        def update_img_preview():
            nonlocal img_preview_lbl
            if img_preview_lbl:
                img_preview_lbl.destroy()
            if temp_img_data["val"]:
                try:
                    img_bytes = base64.b64decode(temp_img_data["val"])
                    img = Image.open(BytesIO(img_bytes))
                    img.thumbnail((120, 120), Image.Resampling.LANCZOS)
                    tk_img = ImageTk.PhotoImage(img)
                    img_preview_lbl = tk.Label(edit_win, image=tk_img, bg="#ffffff")
                    img_preview_lbl.image = tk_img
                    img_preview_lbl.pack(pady=5)
                except Exception:
                    pass
        
        tk.Button(img_f, text="📁 选择图片", command=select_img, width=10,
                 font=self.fonts['small'], relief="flat", cursor="hand2",
                 bg="#e6f7ff", fg="#1677ff").pack(side="left", padx=4)
        tk.Button(img_f, text="📋 粘贴图片", command=paste_img, width=10,
                 font=self.fonts['small'], relief="flat", cursor="hand2",
                 bg="#e6f7ff", fg="#1677ff").pack(side="left", padx=4)
        tk.Button(img_f, text="🗑️ 清除图片", command=clear_img, width=10,
                 font=self.fonts['small'], relief="flat", cursor="hand2",
                 bg="#fff2f0", fg="#ff4d4f").pack(side="left", padx=4)
